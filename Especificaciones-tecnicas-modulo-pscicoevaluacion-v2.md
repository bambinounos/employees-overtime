...
**Versión:** 1.1  
**Fecha:** 2026-02-26  
**Cambios v1.1:** Banco ampliado con selección aleatoria, escala de deseabilidad social, control de consistencia  
...
│   │       ├── consentimiento.html        # Consentimiento informado LOPD (obligatorio)
...
class Prueba(models.Model):
    ...
    items_banco = models.IntegerField(default=0,
        help_text="Total de preguntas en el banco para esta prueba")
    items_a_aplicar = models.IntegerField(default=0,
        help_text="Cantidad de preguntas que se seleccionan aleatoriamente por evaluación. 0 = aplicar todas")
...
class Pregunta(models.Model):
    ...
    par_consistencia = models.ForeignKey('self', on_delete=models.SET_NULL,
        ...
        help_text="Pregunta par para control de consistencia. Si esta pregunta "
                  "tiene par, ambas miden lo mismo con diferente redacción")
    orden = models.IntegerField(default=0)
...
class Evaluacion(models.Model):
    ...
    preguntas_seleccionadas = models.JSONField(null=True, blank=True,
        help_text="Dict {tipo_prueba: [lista de IDs de preguntas seleccionadas]}. "
                  "Se genera al iniciar la evaluación con selección aleatoria del banco")
...
class ResultadoFinal(models.Model):
    ...
    # Control de validez
    puntaje_deseabilidad_social = models.FloatField(null=True,
        help_text="Escala 1-5. >4.0 indica posible falseo de respuestas")
    indice_consistencia = models.FloatField(null=True,
        help_text="0-100%. <60% indica respuestas inconsistentes/aleatorias")
    evaluacion_confiable = models.BooleanField(default=True,
        help_text="False si deseabilidad social >4.0 O consistencia <60%")
...
### 4.0 Estrategia de Banco y Selección Aleatoria

Cada prueba psicométrica tiene un **banco de preguntas amplio** del cual se selecciona aleatoriamente un subconjunto por cada evaluación. Esto proporciona:

1. **Anti-filtración:** Si un candidato comparte las preguntas, al siguiente le salen diferentes
2. **Re-evaluación:** Se puede evaluar al mismo candidato después sin repetir preguntas
3. **Fiabilidad estadística:** Suficientes ítems por dimensión para resultados confiables (α ≥ 0.75)

**Regla de selección:** La selección aleatoria se hace **balanceada por dimensión**, no al azar puro. Es decir, si Big Five tiene 24 ítems por dimensión en el banco y se aplican 12, se seleccionan exactamente 12 de cada dimensión (no 20 de una y 4 de otra).

**Tabla resumen de banco vs aplicación:**

| Prueba | Banco total | Aplicadas por evaluación | Por dimensión (banco→aplicadas) | Tiempo estimado |
|---|---|---|---|---|
| Big Five (OCEAN) | **120** | **60** | 24→12 por cada 5 dimensiones | 15-20 min |
| Compromiso (Allen&Meyer) | **48** | **24** | 16→8 por cada 3 subdimensiones | 8-10 min |
| Obediencia/Conformidad | **40** | **20** | 14→7 / 14→7 / 12→6 | 6-8 min |
| Deseabilidad Social | **12** | **6** | 12→6 (dimensión única) | 2-3 min |
| Situacional | **30** | **15** | 10→5 por cada 3 dimensiones | 15-20 min |
| Frases Sacks | **50** | **30** | ~17→10 por cada 3 dimensiones | 15-20 min |
| **Subtotal psicométricas** | **300** | **155** | | **~60-80 min** |
| Memoria (progresiva) | N/A | ~10-15 niveles | Adaptativa (no aleatorizada) | 8-10 min |
| Matrices progresivas | **30** | **20** | Dificultad progresiva | 20 min |
| Árbol (proyectiva) | N/A | 1 dibujo | — | 5-10 min |
| Persona bajo lluvia | N/A | 1 dibujo | — | 5-10 min |
| Colores (Lüscher) | N/A | 2 rondas de 8 colores | — | 3-5 min |
| **TOTAL ESTIMADO** | | | | **~2 horas** |

Adicionalmente se incluyen **8 pares de consistencia** (16 preguntas ocultas dentro de las pruebas existentes).

### 4.0.1 Control de Consistencia (Pares Duplicados)

Se insertan **8 pares de preguntas** que miden exactamente lo mismo pero con redacción diferente. Están distribuidas dentro de las pruebas normales (el candidato no sabe que son pares). Se aplican **siempre** (no son aleatorias).

**Ejemplo de par:**
- Pregunta 14 (Big Five): "No me voy a casa sin terminar mis pendientes"
- Pregunta 47 (Big Five): "Dejar tareas incompletas al final del día me genera mucha ansiedad"

Si el candidato responde 5 en una y 2 en la otra, hay inconsistencia.

**Cálculo del Índice de Consistencia:**
```python
def calcular_consistencia(evaluacion):
    """
    Compara respuestas en pares de consistencia.
    Retorna 0-100% donde 100% = perfectamente consistente.
    """
    pares = Pregunta.objects.filter(
        par_consistencia__isnull=False,
        id__in=evaluacion.respuestas_psicometricas.values_list('pregunta_id', flat=True)
    ).select_related('par_consistencia')
    
    if not pares.exists():
        return 100.0  # Sin pares para evaluar
    
    diferencias = []
    for pregunta in pares:
        try:
            resp_a = evaluacion.respuestas_psicometricas.get(pregunta=pregunta).valor
            resp_b = evaluacion.respuestas_psicometricas.get(pregunta=pregunta.par_consistencia).valor
            
            # Ajustar por inversión si aplica
            if pregunta.es_inversa != pregunta.par_consistencia.es_inversa:
                resp_b = 6 - resp_b
            
            # Diferencia normalizada (0 = idéntica, 1 = máxima diferencia)
            diferencias.append(abs(resp_a - resp_b) / 4.0)  # escala 1-5, max diff = 4
        except RespuestaPsicometrica.DoesNotExist:
            continue
    
    if not diferencias:
        return 100.0
    
    consistencia = (1 - mean(diferencias)) * 100
    return round(consistencia, 1)
```

**Interpretación:**
- **≥80%**: Respuestas consistentes y confiables
- **60-79%**: Consistencia moderada, revisión recomendada
- **<60%**: Respuestas inconsistentes → **evaluación NO confiable** (posible respuestas al azar, falta de atención, o intento de manipulación)

### 4.0.2 Algoritmo de Selección Aleatoria de Preguntas
...
def seleccionar_preguntas_evaluacion(evaluacion):
    """
    Genera la selección aleatoria de preguntas para una evaluación.
    Se ejecuta UNA VEZ al momento de crear la evaluación.
    Almacena los IDs en evaluacion.preguntas_seleccionadas.
    """
    seleccion = {}
    
    for prueba in Prueba.objects.filter(activa=True).order_by('orden'):
        if prueba.items_a_aplicar == 0:
            # Aplicar todas (ej: memoria, proyectivas)
            ids = list(prueba.preguntas.values_list('id', flat=True))
            seleccion[prueba.tipo] = ids
            continue
        
        # Selección balanceada por dimensión
        dimensiones = (prueba.preguntas
            .values_list('dimension', flat=True)
            .distinct())
        
        ids_seleccionados = []
        items_por_dimension = prueba.items_a_aplicar // len(dimensiones)
        
        for dim in dimensiones:
            preguntas_dim = list(prueba.preguntas
                .filter(dimension=dim)
                .values_list('id', flat=True))
            muestra = random.sample(preguntas_dim,
                min(items_por_dimension, len(preguntas_dim)))
            ids_seleccionados.extend(muestra)
        
        # Agregar pares de consistencia (siempre se incluyen)
        pares = prueba.preguntas.filter(par_consistencia__isnull=False)
        for p in pares:
            if p.id not in ids_seleccionados:
                ids_seleccionados.append(p.id)
            if p.par_consistencia_id not in ids_seleccionados:
                ids_seleccionados.append(p.par_consistencia_id)
        
        # Mezclar orden
        random.shuffle(ids_seleccionados)
        seleccion[prueba.tipo] = ids_seleccionados
    
    evaluacion.preguntas_seleccionadas = seleccion
...
**Banco:** 120 ítems (24 por dimensión), escala Likert 1-5  
**Aplicadas por evaluación:** 60 ítems (12 por dimensión, seleccionadas aleatoriamente)  
**Tiempo:** Sin límite (registrar tiempo por respuesta)  
**Distribución:** 12 ítems por dimensión, mezclados aleatoriamente  
**Incluye:** 2 pares de consistencia ocultos (4 ítems extra fijos)  

**Composición del banco por dimensión (24 ítems c/u):**
- ~14 ítems directos + ~10 ítems inversos por dimensión
- Redactados con diferente nivel de intensidad para capturar gradientes

**Ejemplos de ítems de Responsabilidad (directos):**
...
- "Hago listas de pendientes y las sigo rigurosamente"

**Ejemplos de ítems de Responsabilidad (inversos):**
- "A veces dejo las cosas para el último momento"
- "Suelo olvidar compromisos menores"
- "No me preocupa mucho si dejo algo sin terminar un día"
- "Soy flexible con mis propios plazos"
...
**Banco:** 48 ítems (16 por subdimensión), escala Likert 1-5  
**Aplicadas por evaluación:** 24 ítems (8 por subdimensión, aleatorias)  
**Incluye:** 2 pares de consistencia ocultos (4 ítems extra fijos)  
**Subdimensiones:**
- Compromiso Afectivo (banco: 16, aplica: 8): Apego emocional a la organización
- Compromiso de Continuidad (banco: 16, aplica: 8): Percepción del costo de irse
- Compromiso Normativo (banco: 16, aplica: 8): Sentido de obligación moral
...
**Banco:** 40 ítems, escala Likert 1-5  
**Aplicadas por evaluación:** 20 ítems (aleatorias, balanceadas por subdimensión)  
**Incluye:** 2 pares de consistencia ocultos (4 ítems extra fijos)  
**Subdimensiones:**
- Disciplina (banco: 14, aplica: 7)
- Conformidad normativa (banco: 14, aplica: 7)  
- Orientación a la autoridad (banco: 12, aplica: 6)
...
### 4.4 Escala de Deseabilidad Social (Control de Validez) — NUEVA

**Objetivo:** Detectar si el candidato está mintiendo o exagerando sus respuestas para causar buena impresión. Este es un mecanismo anti-falseo crítico.  
**Banco:** 12 ítems, escala Likert 1-5  
**Aplicadas por evaluación:** 6 ítems (aleatorias)  
**Nota:** Los ítems se intercalan dentro de las otras pruebas psicométricas, NO se presentan como una prueba separada al candidato.

**Diseño de ítems:** Afirmaciones que son socialmente deseables pero estadísticamente improbables de ser ciertas. Una persona honesta marcará 2-3; una persona que falsea marcará 5.

**Ejemplos:**
- "Nunca he llegado tarde a ningún compromiso en mi vida"
- "Jamás he dicho una mentira, ni siquiera una pequeña"
- "Nunca he sentido envidia de los logros de otra persona"
- "Siempre mantengo la calma, sin importar la situación"
- "Nunca he tenido un pensamiento negativo sobre un compañero de trabajo"
- "Jamás he cometido un error en mi trabajo"
- "Siempre he cumplido todas las promesas que he hecho"
- "Nunca he deseado tener algo que pertenece a otra persona"

**Scoring:**
```python
def calcular_deseabilidad_social(respuestas):
    """
    Promedio de respuestas en escala 1-5.
    Interpretación:
      1.0-2.5: Persona muy autocrítica o con baja autoestima
      2.5-3.5: Rango normal y honesto
      3.5-4.0: Tendencia a presentarse bien (normal en contexto laboral)
      >4.0: ALERTA - Probable falseo de respuestas
    """
...
- Si puntaje de deseabilidad social > 4.0 → `evaluacion_confiable = False`
- El resultado se marca con advertencia: "Las respuestas del candidato muestran alta deseabilidad social. Los puntajes de pruebas psicométricas pueden estar inflados."
- No invalida automáticamente al candidato, pero el evaluador recibe la alerta prominente

### 4.5 Test de Memoria de Trabajo (Cognitiva)
...
**Banco:** No aplica selección aleatoria (es adaptativa/progresiva)  
**Niveles aplicados:** 10-15 (depende del rendimiento del candidato)  
...
### 4.6 Matrices Progresivas (Cognitiva - tipo Raven)

**Objetivo:** Medir inteligencia no verbal / razonamiento lógico.  
**Banco:** 30 matrices con patrón a completar, 4 opciones de respuesta c/u  
**Aplicadas por evaluación:** 20 matrices (seleccionadas manteniendo progresión de dificultad)  
...
### 4.7 Test del Árbol - Karl Koch (Proyectiva)
...
### 4.8 Persona bajo la Lluvia (Proyectiva)
...
### 4.9 Frases Incompletas de Sacks (Proyectiva)

**Objetivo:** Revelar actitudes profundas hacia trabajo, autoridad y compromiso.  
**Banco:** 50 frases incompletas  
**Aplicadas por evaluación:** 30 frases (10 por dimensión, seleccionadas aleatoriamente)  
**Tiempo:** Sin límite  
...
### 4.10 Test de Colores - Lüscher Simplificado (Proyectiva)
...
### 4.11 Prueba Situacional (Psicométrica)

**Objetivo:** Evaluar reacción real ante escenarios laborales específicos.  
**Banco:** 30 escenarios con 4 opciones de respuesta cada uno  
**Aplicadas por evaluación:** 15 escenarios (5 por dimensión, aleatorios)  
...
def calcular_resultado_final(evaluacion):
    ...
    # 0. Controles de validez (PRIMERO)
    resp_deseabilidad = evaluacion.respuestas_psicometricas.filter(
        pregunta__prueba__tipo='DESEABILIDAD')
    resultado.puntaje_deseabilidad_social = calcular_deseabilidad_social(resp_deseabilidad)
    resultado.indice_consistencia = calcular_consistencia(evaluacion)
    resultado.evaluacion_confiable = (
        resultado.puntaje_deseabilidad_social <= 4.0 and
        resultado.indice_consistencia >= 60.0
    )
...
def determinar_veredicto(resultado, perfil):
    """
    APTO: Cumple TODOS los umbrales mínimos + evaluación confiable
    NO APTO: Falla en 2+ criterios críticos
    REVISION: Falla en 1 criterio, proyectivas pendientes, o evaluación no confiable
    """
    # Si la evaluación no es confiable, siempre va a REVISIÓN
...
[Admin genera link] 
    ...
    → Consentimiento informado (LOPD Ecuador)
    → Instrucciones generales
    → [SISTEMA: Selección aleatoria de preguntas del banco → se almacena en evaluacion.preguntas_seleccionadas]
    
    → PRUEBA 1: Big Five (60 preguntas Likert + 6 de deseabilidad social intercaladas)
    → PRUEBA 2: Compromiso Organizacional (24 preguntas Likert)
    → PRUEBA 3: Obediencia (20 preguntas Likert)
    → PRUEBA 4: Test de Memoria (interactivo JS, adaptativo ~10-15 niveles)
    → PRUEBA 5: Matrices Progresivas (20 matrices, 20 min máx)
    ...
    → PRUEBA 9: Test de Colores (drag & drop, 2 rondas)
    → PRUEBA 10: Prueba Situacional (15 escenarios)
    
    → Pantalla de finalización ("Gracias, sus resultados serán revisados")
    → Token se invalida
    
NOTA: Los 6 ítems de Deseabilidad Social se intercalan dentro de la
Prueba 1 (Big Five) de forma invisible. Los 8 pares de consistencia
están distribuidos dentro de las Pruebas 1, 2 y 3.
...
2. Implementar todos los modelos (incluyendo `par_consistencia`, `preguntas_seleccionadas`, campos de deseabilidad)
3. Migraciones
4. Admin Django básico
5. Management command `seed_pruebas` con banco completo:
   - 120 ítems Big Five (24 por dimensión, ~40% inversos)
   - 48 ítems Compromiso (16 por subdimensión)
   - 40 ítems Obediencia (balanceados por subdimensión)
   - 12 ítems Deseabilidad Social
   - 30 escenarios Situacionales (10 por dimensión)
   - 50 frases Sacks (balanceadas por dimensión)
   - 8 pares de consistencia vinculados con `par_consistencia`
   - 30 matrices progresivas con SVG propios
6. Management command `seleccionar_preguntas` para generar selección aleatoria
...
1. Motor de scoring automático (incluyendo deseabilidad social + consistencia)
2. Generación de reportes PDF (con indicadores de confiabilidad)
3. Vista comparativa de candidatos
4. Alertas visuales cuando `evaluacion_confiable = False`
...
### 12.5 Banco de preguntas y fiabilidad
El banco total contiene ~330 ítems de los cuales se aplican ~155 por evaluación. Con 12 ítems por dimensión en Big Five, se espera una fiabilidad (alfa de Cronbach) de α ≥ 0.75. A medida que se acumulen datos reales de candidatos, se recomienda calcular el alfa real y descartar ítems con baja correlación ítem-total. También se puede hacer análisis de ítems para identificar preguntas que no discriminan bien y reemplazarlas.

### 12.6 Deseabilidad social y contexto laboral
Es normal que en contexto de selección laboral los candidatos se presenten ligeramente mejor de lo que son (puntaje 3.0-3.5 en deseabilidad). Solo puntajes > 4.0 deben generar alerta. El evaluador debe considerar que un puntaje de 4.0+ no significa que el candidato sea "malo", sino que sus resultados en las pruebas Likert pueden estar inflados y necesitan interpretarse con cautela cruzando con las pruebas proyectivas y situacionales (que son más difíciles de falsear).
